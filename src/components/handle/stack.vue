...
// pages数据包含基础的图片数据
        ...
        methods: {
            ...
            transform (index) {
                ...
                if
                (index >= this.basicdata.currentPage) {
                    let style = {};
...
<script type="text/ecmascript-6">
    import detectPrefixes from '@/assets/js/detect-prefixes.js'
    export default {
        props: {
            stackinit: {//可见几个
                type: Object,
...
//                初始位置信息，滑动式改变
                temporaryData: {
                    ...
                    rotate: 0,
                    lastRotate: 0,
                    visible: this.stackinit.visible || 3,//可视几个，默认3个
                    tracking: false,//是否滑动开始
                    animation: false,
            ...
            touchstart (e) {
                ...
                if (e.type === 'touchstart') {
                    if (e.touches.length > 1) {
                        ...
                        // 记录起始位置
                        this.basicdata.start.t = new Date().getTime();//开始滑动时间
...
//                        移动端位置
                    ...
                    let rotateDirection = this.rotateDirection();//横向移动方向
                    let angleRatio = this.angleRatio();//纵向偏移率
//                    偏移率=横向移动方向*划出宽度比例*15*纵向偏移比例
...
//            纵向偏移率
            angleRatio () {
                let height = this.$el.offsetHeight;
                let offsetY = this.temporaryData.offsetY;//新偏移坐标
                let ratio = -1 * (2 * offsetY / height - 1);
                return ratio || 0
            },
            inStack (index, currentPage) {
                let stack = [];
                let visible = this.temporaryData.visible;//可见页
                let length = this.pages.length;//总长度
            ...
            transformIndex (index) {
                if (index === this.temporaryData.currentPage) {
                    let style = {};
                    style['transform'] = 'translate3D(' + this.temporaryData.poswidth + 'px' + ','
                            + this.temporaryData.posheight + 'px' + ',0px) ' + 'rotate(' + this.temporaryData.rotate + 'deg)';
